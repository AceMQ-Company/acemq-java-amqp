#!/usr/bin/env bash
# Render docs/*.md into site/, and fold in the aggregated Javadoc.
#
...
# markdown said, and every cross-page link in this repository was written as
# `.html` for months: dead in GitHub's markdown view, invisible to every check,
# and found by somebody reading the docs in the repository rather than by the
# build.
#
# It runs first because it needs neither pandoc nor the javadoc, and a bad link
# is cheapest to find before a minute of rendering.
python3 - <<'PY'
...
for page in pages:
    ...
    for target in re.findall(r"\]\(([^)\s]+)\)", body):
        ...
        if not path:
            continue
        links += 1
        # The generated API reference is genuinely HTML: javadoc writes it, no
        # markdown renders into it, and it is .html on GitHub and on the site
        # alike. Any other .html target is a docs page written the wrong way
        # round -- it is the mistake this check exists for, so it is named as
        # that rather than reported as a missing file.
...
# Javadoc. The aggregate goal has written to target/site/apidocs historically and
# to target/reports/apidocs in recent plugin versions, so both are accepted --
# and a missing one is fatal rather than silently shipping a site whose API
# reference 404s.
APIDOCS=""
...
if [ -z "$APIDOCS" ]; then
  echo "no aggregated javadoc found; run: mvn -DskipTests javadoc:aggregate" >&2
  exit 1
fi
mkdir -p "$OUT/apidocs"
